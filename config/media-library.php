...
return [
     ...
     */
    // Raised from the package default of 10MB to accommodate the Home page
    // video hero (App\Models\HomeVideoHero, 'video' collection). Filament's
    // SpatieMediaLibraryFileUpload fields still set their own per-field
    // ->maxSize() (in KB) so an oversized upload gets a clean inline error
    // instead of Spatie's uncaught FileIsTooBig exception — see
    // CLAUDE.md "Hard rule … media uploads". Note: on the production server,
    // PHP's own upload_max_filesize / post_max_size must be >= this too.
